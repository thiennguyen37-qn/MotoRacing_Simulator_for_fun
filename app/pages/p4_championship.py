...
class ChampionshipPage(QWizardPage):
    def __init__(self, wiz):
        super().__init__()
        self._wiz = wiz
        # No page title/subtitle and no info line — the standings tables are the
        # whole page, so nothing sits above them.
    ...
    def handle_key(self, key: int) -> bool:
        ...
        if self._in_content:
            ...
            if key in (K.Key_Left, K.Key_Right):
                # swallowed on purpose: inside a standings table ←/→ must not
                # jump back out to the tab/button row (Esc does that)
